...
- Even a dealer's own listed mileage can be stale/wrong, not just aggregator mileage — confirmed on Audi South Atlanta (Union City, GA), where the dealer's own site said 25,368mi but Stiles's own photo of the car's actual odometer showed 27,424mi (2,056mi understated), likely just a stale listing that hasn't been refreshed since it was first posted. An in-person or phone-confirmed odometer reading always wins over any listed figure, dealer or aggregator.
- A listed price that's wildly out of line (e.g. ~2x every comparable listing, near or above MSRP for a used car) is more likely a parsing error than a real price — some dealer sites display a "retail price"/MSRP figure prominently alongside a discount and the actual selling price, and it's easy to grab the wrong one. Confirmed on Valenti Audi's site (Watertown, CT): Visor had pulled $149,889 for a car actually priced at $73,598. Flag price anomalies for a human check (dealer page or phone call) rather than excluding on price alone or trusting the anomalous figure.
- Once a VIN is excluded as confirmed sold/delisted (verified directly against the dealer's own page), don't reinstate it just because an aggregator (Visor, Edmunds, CarGurus, Cars.com) still shows it active — those caches lag real dealer inventory by days to weeks. Confirmed on Auto Web Expo's Plano, TX listing (Aug 2026): Edmunds still showed it "active" at a stale price days after the dealer's own site had dropped the VIN entirely. Only reinstate a sold-excluded VIN with a fresh dealer-page or phone confirmation that it's live again.
...
- One small piece of CI does exist: `.github/workflows/merge-claude-branches.yml`. The Claude Code Cloud Routine that runs `ROUTINE.md` daily cannot push straight to `main` — Anthropic's cloud platform redirects its pushes to a `claude/*` branch as a safety default, regardless of what the routine's own prompt says. This workflow merges the instant one of those branches appears, so the routine still ends up fully unattended end-to-end. It doesn't build or compile anything — it only merges. Uses `git merge --no-ff` rather than a strict fast-forward, so it doesn't fail just because `main` picked up an interactive push after the routine's branch was cut (this happened once, Aug 12 2026 — self-healed on the next run, but `--no-ff` closes the gap so it doesn't need a lucky retry). A genuine content conflict still fails the job on purpose, since that needs a human.
