...
- The site also has one serverless function, `netlify/functions/update-listing.js`, backing the ✎ edit icon next to each dealer link in the UI. It takes a pasted URL (+ optional image URL), tries to pull an `og:image` from the new page server-side, and commits the updated link/photo straight to `main` via the GitHub API — using a `GT_TRACKER_GITHUB_TOKEN` env var (a fine-grained PAT, Contents read/write, scoped to just this repo). No login gate on the page itself: anyone with the link can use the edit feature, by deliberate choice given this is a low-stakes personal tool. `index.html` is still hand-edited HTML with a `data-id` per card (matching each card's image filename stem) — the function does a targeted string replace on that one card's block, not a full rewrite.
